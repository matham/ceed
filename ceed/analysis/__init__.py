...
from ceed.utils import CeedWithID
...
class CeedDataReader:
    ...
    :attr:`shapes_intensity_rendered_gpu_rate` additionally, breaks the longer
    frames into individual frames so it's generally the same size as
    :attr:`shapes_intensity`.
    ...
    _shapes_intensity_rendered: Optional[Dict[str, np.ndarray]] = None

    @property
    def shapes_intensity_rendered(self) -> Dict[str, np.ndarray]:
        """After :meth:`load_experiment`, it is a mapping whose keys are the
        names of the shapes drawn in ceed for this experiment and whose values
        is a 2D array containing the intensity value of the shape for each Ceed
        ...
        Long frames, frames displayed for more than one GPU frame period
        ...
        because the duration of each item may be longer than the GPU period).
        """
        rendered = self._shapes_intensity_rendered
        if rendered is None:
            rendered_frames = self.rendered_frames
            rendered = self._shapes_intensity_rendered = {}
            for name, item in self.shapes_intensity.items():
                rendered[name] = item[rendered_frames, :]

        return rendered

    _shapes_intensity_rendered_gpu_rate: Optional[Dict[str, np.ndarray]] = None

    @property
    def shapes_intensity_rendered_gpu_rate(
            self) -> Optional[Dict[str, np.ndarray]]:
        """Like :attr:`shapes_intensity_rendered`, except that samples
        ...
        In this example, :attr:`shapes_intensity` shapes would hold 5 values
        corresponding to frames 0, 1, 2, 3, 4, :attr:`shapes_intensity_rendered`
        shapes would hold 4 values corresponding to frames 0, 1, 2, 4, and
        :attr:`shapes_intensity_rendered_gpu_rate` shapes would hold 5 values
        corresponding to frames 0, 1, 1, 2, 4. :attr:`rendered_frames_long`
        would be an array with values 1, 2, 1, 1.

        :attr:`shapes_intensity_rendered_gpu_rate` is None when
        :attr:`electrode_intensity_alignment` is None.
        ...
        if rendered_gpu_rate is None:
            long_frames = self.rendered_frames_long
            if long_frames is None:
                return None

            rendered_gpu_rate = self._shapes_intensity_rendered_gpu_rate = {}

            for name, item in self.shapes_intensity_rendered.items():
                rendered_gpu_rate[name] = np.repeat(item, long_frames, axis=0)

        return rendered_gpu_rate
    ...
    _rendered_frames_long: Optional[np.ndarray] = None

    @property
    def rendered_frames_long(self) -> Optional[np.ndarray]:
        """After :meth:`load_experiment` it is a 1D array of the same
        size as :attr:`shapes_intensity_rendered` indicating the number of GPU
        periods for which the corresponding frame was rendered. If it's larger
        than one it means the frame was long. See :attr:`shapes_intensity`
        and :attr:`shapes_intensity_rendered_gpu_rate`.

        :attr:`rendered_frames_long` is None when
        :attr:`electrode_intensity_alignment` is None.
        ...
        .. warning::
            ...
            expected GPU period) it is not accurate.
        """
        long_frames = self._rendered_frames_long
        if long_frames is None:
            alignment = self.electrode_intensity_alignment
            if alignment is None:
                return None
    ...
    * the remaining 3 columns is the state of the R, G, and B LED as set by
    ...
    def format_event_data(
            ...
            ceed_name: Optional[str] = None, ceed_id: Optional[int] = None
    ) -> list:
        """Returns a flattened and optionally filtered version of the
        :attr:`event_data`.

        Unlike :attr:`event_data` where each event item is a 4-tuple of
        ``(frame, ceed_id, name, payload)`` and ``payload`` is a list of
        event data. Each item in the returned list is a list whose first three
        items are ``[frame, obj, name, ...]`` and the remaining items is the
        payload. That is the payload items are appended to the list.

        Notice how we return ``obj``, rather than ``ceed_id``. The ``obj`` is
        the stage or function object that emitted the event.

        :param event: If not None, the name of the event to such that only these
            events will be returned. E.g. ``"loop_start"``.
        :param ceed_obj: If not None, it's an object such that only events
            generated by this object is returned. This could be e.g. a stage or
            function.
        :param ceed_name: If not None, it's a name such that only events
            generated by an object of this name is returned. This could be e.g.
            the name of a stage or function.
        :param ceed_id: If not None, it's the
            :attr:`~ceed.utils.CeedWithID.ceed_id` such that only events
            generated by an object with this ceed id is returned.

        E.g. if :attr:`event_data` is e.g.
        ``[(300, 1, 'start', [0, 1, 0.33])]``, then without any filtering this
        method returns ``[[300, func_obj, 'start', 0, 1, 0.33]]``.
        """
        ids_map = {s.ceed_id: s for s in self.experiment_stage.get_stages()}
        f_ids_map = {f.ceed_id: f for f in self.experiment_stage.get_funcs()}
        ids_map.update(f_ids_map)

        if ceed_obj is not None:
            ceed_id = ceed_obj.ceed_id

        events = []
        if ceed_name is not None:
            for frame, id_, event_name, payload in self.event_data:
                obj = ids_map[id_]
                if obj.name == ceed_name and \
                        (event is None or event_name == event):
                    events.append([frame, obj, event_name, *payload])
        elif ceed_id is not None:
            for frame, id_, event_name, payload in self.event_data:
                if id_ == ceed_id and (event is None or event_name == event):
                    events.append([frame, ids_map[id_], event_name, *payload])
